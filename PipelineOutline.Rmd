...
This is intended as an example pipeline for a project. There's definitely ways to improve the efficiency of this process, but it does in fact work. It is likely best to use this pipeline as a reference in conjuction with the other resources on the website and your own best judgement. The steps begin assuming you have two .fastq.gz files (a R1 and R3) for each plate (as in commonly how the Miller Lab will return files to you). If that's not the case, go check out the Downloading Data tab for more details on earlier steps. 
...
**Note** This is per plate for simplicity sake. For a much slicker way of doing it, see the Downloading Data tab and associated files. 

First, we will split our .fastq files for each plate using the `run_BestRadSplit_PstI.sh` `bash` script provided by the Miller Lab. 

##### Note: The script is specific to the PstI cutter, so use SbfI if that is the cutter, etc. (See Downloading Data tab for more info)
...
In a new screen, compress the output of the `split` script using `gzip`. 

### The `align` script

Next, we will make two lists of all of the .fastq.gz file names (they should be .gz because we compressed them!) and then paste them together. We'll then feed this list to a `run_align` script with a reference genome (available online). 
...
It's also good to check the length of the bams file (should be 96 per plate) using `ls filtered_bams/ | wc`. 
